...
NB_MODULE(mav_trajectory_generation_py, m) {
    ...
    nb::class_<PolyOptNonLin<10>>(m, "PolynomialOptimizationNonLinear")
        ...
        .def("get_trajectory",
             [](PolyOptNonLin<10> &opt, Trajectory &traj) {
                 opt.getTrajectory(&traj);
             },
             nb::arg("trajectory"))
        .def("get_optimization_info",
             &PolyOptNonLin<10>::getOptimizationInfo)
      ;
    ...
    // === nlopt helper ===
     m.def("nlopt_return_value_to_string",
          [](nlopt::algorithm v){ return nlopt::returnValueToString(v); },
          nb::arg("return_value"));
}
